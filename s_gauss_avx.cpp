    ...
    void xorbit(bitmap b)
    {
        ...
        for(;i<=bytenum-8;i+=8)
            //对两行的每一位都进行异或操作
            {
                //注意，均为整型
                __m256 t1,t2,t3;
                //t1=_mm256_load_si128((__m128i*)bitVector+i);
                t1=_mm256_loadu_ps((float*)bitVector+i);
                t2=_mm256_loadu_ps((float*)b.bitVector+i);
                t3=_mm256_xor_ps(t1,t2);
                _mm256_storeu_ps((float*)bitVector+i,t3);
                //t2=_mm256_load_si128((__m128i*)b.bitVector+i);
                //t3=_mm256_xor_si128(t1,t2);
                //_mm256_store_si128((__m128i*)bitVector+i,t3);

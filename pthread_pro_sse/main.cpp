...
//给定矩阵的列数、被消元行数
string str1="D:\\bingxing\\homework-X86\\special\\data\\被消元行6.txt";
string str2="D:\\bingxing\\homework-X86\\special\\data\\消元子6.txt";
int bytenum=(n+31)/32;
//32位组成一组，一共有的组数
typedef struct
{
        ...
        if(!eliminator[i].isnull())
        {
            for(int j=t_id;j<elinenum;j+=NUM_THREADS)
            {
                ...
                if(eline[j].getcol()==i)
                {
                    ...
                    {
                        __m256 t1,t2,t3;
                        t1=_mm256_loadu_ps((float*)(eline[j].bitVector+k));
                        t2=_mm256_loadu_ps((float*)(eliminator[i].bitVector+k));
                        t3=_mm256_xor_ps(t1,t2);
                        _mm256_storeu_ps((float*)(eline[j].bitVector+k),t3);
